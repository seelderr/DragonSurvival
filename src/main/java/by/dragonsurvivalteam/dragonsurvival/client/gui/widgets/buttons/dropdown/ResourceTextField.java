	...
	public List<ResourceEntry> parseCombinedList(final List<String> values, boolean isTag) {
		...
		for (String value : values) {
			...
			if (isTag) {
				...
				if (isEntity) {
                    try {
                        results.add(new ResourceEntry(value, Objects.requireNonNull(ForgeRegistries.ENTITIES.tags().getTag(TagKey.create(Registry.ENTITY_TYPE_REGISTRY, location))).stream().map(type -> new ItemStack(ForgeSpawnEggItem.fromEntityType(type))).toList(), true));
                    } catch (Exception e) {
			...
			if (isBlock) {
                try {
                    results.add(new ResourceEntry(value, Collections.singletonList(new ItemStack(new BlockStateParser(new StringReader(value), false).parse(false).getState().getBlock()))));
                } catch (CommandSyntaxException ignored) { /* Nothing to do */ }
            }
