	...
	public static void drawAbilityHover(PoseStack stack, int xPos, int yPos, DragonAbility ability){
		...
		if(!description.isEmpty()){
			if(ability.getInfo().size() > 0){
				...
				GuiUtils.drawContinuousTexturedBox(stack, BARS, xPos - (Screen.hasShiftDown() ? extraWidth : 10), yPos + 3, 40, 20, Screen.hasShiftDown() ? extraWidth : 15, Screen.hasShiftDown() ? Math.min(27 + text.size() * 9, 35 + 24 + description.size() * 9 - 10) : 27 + text.size() * 9, 20, 20, 3,  (float)0);
				GuiUtils.drawContinuousTexturedBox(stack, BARS, xPos - (Screen.hasShiftDown() ? extraWidth : 10) + 3, yPos + 9, colorXPos, colorYPos, Screen.hasShiftDown() ? extraWidth : 15, 20, 20, 20, 3, 0);

				if(Screen.hasShiftDown()){
					Gui.drawString(stack, Minecraft.getInstance().font, new TranslatableComponent("ds.skill.info"), xPos - extraWidth + 10, yPos + 15, -1);
			...
			GuiUtils.drawContinuousTexturedBox(stack, BARS, xPos - 2, yPos - 4, 40, 20, 150 + 5, 35 + 24 + description.size() * 9, 20, 20, 3, 3, 3, 3, (float)0);
		}

		GuiUtils.drawContinuousTexturedBox(stack, BARS, xPos, yPos + 3, colorXPos, colorYPos, 150, 20, 20, 20, 3, 0);
		GuiUtils.drawContinuousTexturedBox(stack, BARS, xPos, yPos, 0, 100, 26, 26, 24, 24, 3, 0);
		...
		if(skillType != null){
			...
			Gui.drawCenteredString(stack, Minecraft.getInstance().font, new TranslatableComponent("ds.skill.type." + skillType), xPos + 150 / 2, yPos + 30, c.getColor());
		}

		if(ability.getMaxLevel() > 1){
			Gui.drawCenteredString(stack, Minecraft.getInstance().font, new TextComponent(ability.getLevel() + "/" + ability.getMaxLevel()), xPos + 150 - 18, yPos + 9, -1);
		...
		if(ability.getInfo().size() > 0)
			Gui.drawCenteredString(stack, Minecraft.getInstance().font, new TranslatableComponent("ds.skill.info.hold_shift").withStyle(ChatFormatting.DARK_GRAY), xPos + 150 / 2, yPos + 47 + (description.size() - 1) * 9, 0);
