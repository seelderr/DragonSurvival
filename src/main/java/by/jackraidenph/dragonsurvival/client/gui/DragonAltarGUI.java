...
public class DragonAltarGUI extends Screen
{
        ...
        for(GuiEventListener btn : children){
            if(btn instanceof AltarTypeButton){
                ...
                if(button.isHoveredOrFocused()) {
                    ...
                    renderDragon(width / 2 + 170, button.y + (button.getHeight() / 2) + 32, 5, PoseStack , DragonLevel.ADULT.size, FakeClientPlayerUtils.getFakePlayer(0, handler1), FakeClientPlayerUtils.getFakeDragon(0, handler1));
                    renderDragon(width / 2 - 170, button.y + (button.getHeight() / 2) + 32, -5, PoseStack , button.type == DragonType.NONE ? DragonLevel.ADULT.size : DragonLevel.BABY.size, FakeClientPlayerUtils.getFakePlayer(1, handler2), FakeClientPlayerUtils.getFakeDragon(1, handler2));
                }
            }
        }
    
        TextRenderUtil.drawCenteredScaledText(PoseStack , width / 2, 10, 2f, title.getString(), DyeColor.WHITE.getTextColor());
    
        super.render(PoseStack , mouseX, mouseY, partialTicks);
    }
    
    private void renderDragon(int x, int y, int xrot, PoseStack  PoseStack , float size, Player player, DragonEntity dragon)
    {
        PoseStack .pushPose();
    ...
    protected void init() {
        ...
        this.addRenderableWidget(new HelpButton(this.guiLeft, 10, 18, 18, "ds.help.altar"){
            @Override
        ...
        addRenderableWidget(new AltarTypeButton(this, DragonType.CAVE, width / 2 - 104, this.guiTop + 20));
        addRenderableWidget(new AltarTypeButton(this, DragonType.FOREST, width / 2 - 51, this.guiTop + 20));
        addRenderableWidget(new AltarTypeButton(this, DragonType.SEA, width / 2 + 2, this.guiTop + 20));
        addRenderableWidget(new AltarTypeButton(this, DragonType.NONE, width / 2 + 55, guiTop + 20));
        //ds.gui.customization
        addRenderableWidget(new ExtendedButton(width / 2 - 50, height - 25, 100, 20, new TranslatableComponent("ds.gui.customization"), (btn) -> {
