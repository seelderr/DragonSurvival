	...
	public void render(DragonBeaconTileEntity dragonBeaconEntity, float v, PoseStack PoseStack, MultiBufferSource iRenderTypeBuffer, int light, int overlay){
		...
		switch(type){
			case PEACE -> {
				...
				// FIXME: How to add particles when using a basic type?
				if(!minecraft.isPaused() && dragonBeaconEntity.tick % 5 == 0 && hasMemoryBlock){
					//clientWorld.addParticle(DSParticles.PEACE_BEACON_PARTICLE, dragonBeaconEntity.getX() + x, dragonBeaconEntity.getY() + 0.5, dragonBeaconEntity.getZ() + z, 0, 0, 0);
				}
			}
			case MAGIC -> {
				...
				if(!minecraft.isPaused() && dragonBeaconEntity.tick % 5 == 0 && hasMemoryBlock){
					//clientWorld.addParticle(DSParticles.MAGIC_BEACON_PARTICLE, dragonBeaconEntity.getX() + x, dragonBeaconEntity.getY() + 0.5, dragonBeaconEntity.getZ() + z, 0, 0, 0);
				}
			}
			case FIRE -> {
				...
				if(!minecraft.isPaused() && dragonBeaconEntity.tick % 5 == 0 && hasMemoryBlock){
					//clientWorld.addParticle(DSParticles.FIRE_BEACON_PARTICLE, dragonBeaconEntity.getX() + x, dragonBeaconEntity.getY() + 0.5, dragonBeaconEntity.getZ() + z, 0, 0, 0);
				}
			}
		}
