...
public class ClientMagicHUDHandler {
	...
	public static void renderExperienceBar(final DragonStateHandler handler, final ForgeGui gui, final PoseStack poseStack, int width) {
		...
		int i = localPlayer.getXpNeededForNextLevel();
		if (i > 0) {
			int k = (int) (localPlayer.experienceProgress * 183.0F);
			int l = screenHeight - 32 + 3;
			blit(poseStack, x, l, 0, 164, 182, 5);
			if (k > 0)
				blit(poseStack, x, l, 0, 169, k, 5);
		}
		...
		if (localPlayer.experienceLevel > 0) {
			...
			Minecraft.getInstance().font.draw(poseStack, s, (float) (i1 + 1), (float) j1, 0);
			Minecraft.getInstance().font.draw(poseStack, s, (float) (i1 - 1), (float) j1, 0);
	...
	public static void renderAbilityHud(final DragonStateHandler handler, final PoseStack poseStack, int width, int height){
		Player localPlayer = Minecraft.getInstance().player;
		...
		if (handler.getMagicData().shouldRenderAbilities()) {
			RenderSystem.setShaderTexture(0, VANILLA_WIDGETS);
			Screen.blit(poseStack, posX, posY - 2, 0, 0, 0, 41, 22, 256, 256);
			Screen.blit(poseStack, posX + 41, posY - 2, 0, 141, 0, 41, 22, 256, 256);
			...
			for (int x = 0; x < MagicCap.activeAbilitySlots; x++) {
				...
				if (ability != null && ability.getIcon() != null) {
					...
					Screen.blit(poseStack, posX + x * sizeX + 3, posY + 1, 0, 0, 16, 16, 16, 16);
					...
					if (ability.getSkillCooldown() > 0 && ability.getCurrentCooldown() > 0 && ability.getSkillCooldown() != ability.getCurrentCooldown()) {
						...
						Gui.fill(poseStack, boxX, boxY, boxX + 16, boxY + offset, fColor);
					}
				}
			}

			RenderSystem.setShaderTexture(0, VANILLA_WIDGETS);
			Screen.blit(poseStack, posX + sizeX * handler.getMagicData().getSelectedAbilitySlot() - 1, posY - 3, 2, 0, 22, 24, 24, 256, 256);
		...
		if (handler.getMagicData().isCasting) {
			if (currentCastTime > 0 && skillCastTime != -1) {
				...
				Screen.blit(poseStack, startX, startY, 0, yPos1, 196, 47, 256, 256);
				Screen.blit(poseStack, startX + 2, startY + 41, 0, yPos2, (int) (191 * perc), 4, 256, 256);

				RenderSystem.setShaderTexture(0, ability.getIcon());
				Screen.blit(poseStack, startX + 78, startY + 3, 0, 0, 36, 36, 36, 36);
		...
		if (errorTicks > 0) {
			Minecraft.getInstance().font.draw(poseStack, errorMessage, width / 2f - Minecraft.getInstance().font.width(errorMessage) / 2f, height - 70, 0);
			errorTicks--;
