...
public abstract class ConfigScreen extends OptionsSubScreen{
	...
	private void addConfigs(){
		...
		for (ConfigOption configOption : configOptions) {
			...
			Component tooltip = new TextComponent(tooltip0);
			...
			if (Number.class.isAssignableFrom(field.getType())) {
				...
				if (configValue.get() instanceof List<?> listConfig && (listConfig.isEmpty() || listConfig.get(0) instanceof String || listConfig.get(0) instanceof Number)) {
					...
					CycleOption<String> option = new CycleOption<>(
							name,
                            options -> text,
                            (val1, val2, val3) -> minecraft.setScreen(new ConfigListMenu(this, minecraft.options, new TextComponent(name), configValue, screenSide(), configOption)),
                            () -> new Builder<String>(ignored -> new TextComponent(text)).displayOnlyValue().withValues(text).withInitialValue(text)).setTooltip(minecraft -> ignored -> minecraft.font.split(tooltip, 200));
