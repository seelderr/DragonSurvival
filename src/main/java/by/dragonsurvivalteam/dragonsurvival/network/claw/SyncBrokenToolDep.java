...
public record SyncBrokenToolDep(int playerId, int slot) {
    public void encode(final FriendlyByteBuf buffer) {
        buffer.writeInt(playerId);
        buffer.writeInt(slot);
    }

    public static SyncBrokenToolDep decode(final FriendlyByteBuf buffer) {
        return new SyncBrokenToolDep(buffer.readInt(), buffer.readInt());
    }

    public static void handle(final SyncBrokenToolDep message, final Supplier<NetworkEvent.Context> supplier) {
