...
public class AppleSkinSupport{
	...
	public void onClientTick(ClientTickEvent event){
		if(!ConfigHandler.SERVER.customDragonFoods.get() || !ConfigHandler.CLIENT.appleskinSupport.get()){
			return;
		}
	...
	public static void drawSaturationOverlay(float saturationGained, float saturationLevel, Minecraft mc, MatrixStack matrixStack, int right, int top, float alpha){
		if(!(saturationLevel + saturationGained < 0.0F)){
			...
			for(int i = startSaturationBar; i < endSaturationBar; ++i){
				Point offset = foodBarOffsets.get(i);
				if(offset != null){
	...
	private static void generateHungerBarOffsets(int top, int left, int right, int ticks, PlayerEntity player){
		...
		for(int i = 0; i < 10; ++i){
			...
			Point point = foodBarOffsets.get(i);
			if(point == null){
				point = new Point();
		...
		if(!event.isCanceled()){
			...
			if(DragonUtils.isDragon(Minecraft.getInstance().player)){
				if(DragonFoodHandler.isDragonEdible(hoveredStack.getItem(), DragonUtils.getDragonType(Minecraft.getInstance().player))){
					...
					int saturationBars = (int)Math.ceil(Math.abs(biggestSaturationIncrement) / 2.0F);
