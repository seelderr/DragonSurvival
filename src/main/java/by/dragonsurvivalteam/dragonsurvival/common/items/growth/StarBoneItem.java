				...
				if (!worldIn.isClientSide) {
					...
					if (handler.getPassengerId() != 0) {
						...
						if (mount != null) {
							...
							NetworkHandler.CHANNEL.send(PacketDistributor.PLAYER.with(() -> (ServerPlayer) playerIn), new SynchronizeDragonCap(playerIn.getId(), handler.isHiding(), handler.getType(), handler.getBody(), handler.getSize(), handler.hasWings(), 0));
						}
					}
				}
