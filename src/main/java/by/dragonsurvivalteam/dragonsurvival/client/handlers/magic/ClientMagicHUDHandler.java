<<<<<<< HEAD:src/main/java/by/jackraidenph/dragonsurvival/client/handlers/magic/ClientMagicHUDHandler.java
...
public class ClientMagicHUDHandler
{
	...
	public static void cancelExpBar(ForgeIngameGui gui, PoseStack mStack, float partialTicks, int width, int height) {
		...
		if(Minecraft.getInstance().options.hideGui || !gui.shouldDrawSurvivalElements() || !Minecraft.getInstance().gameMode.hasExperience()) return;
		int x = width / 2 - 91;
		...
		if(!ConfigHandler.SERVER.consumeEXPAsMana.get() || !DragonUtils.isDragon(playerEntity)){
			...
			DragonStateProvider.getCap(playerEntity).ifPresent(cap -> {
				...
				if(DragonUtils.getCurrentMana(playerEntity) < ability.getManaCost() && ((DragonUtils.getCurrentMana(playerEntity) + (playerEntity.totalExperience / 10) >= ability.getManaCost()) || playerEntity.experienceLevel > 0)){
...
>>>>>>> v1.16.x:src/main/java/by/dragonsurvivalteam/dragonsurvival/client/handlers/magic/ClientMagicHUDHandler.java
	...
	public static void renderAbilityHud(ForgeIngameGui gui, PoseStack mStack, float partialTicks, int width, int height) {
	    ...
	    DragonStateProvider.getCap(playerEntity).ifPresent(cap -> {
	        ...
	        if(cap.getMagic().renderAbilityHotbar()) {
				...
				int maxMana = DragonUtils.getMaxMana(playerEntity);
				int curMana = DragonUtils.getCurrentMana(playerEntity);
            ...
            if(ability.getCurrentCastTimer() > 0){
	            ...
	            RenderSystem.setShaderTexture(0,ability.getIcon());
	            Screen.blit(mStack, startX + 78, startY + 3,
	                        0, 0, 36, 36, 36, 36);
