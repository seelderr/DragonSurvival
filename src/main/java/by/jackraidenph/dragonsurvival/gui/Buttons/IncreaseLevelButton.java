		...
		DragonStateProvider.getCap(Minecraft.getInstance().player).ifPresent(cap -> {
			...
			if(ability != null) {
				...
				if(currentAbility != null && currentAbility.getLevel() < currentAbility.getMaxLevel()) {
					...
					GuiUtils.drawHoveringText(stack, Arrays.asList(new TranslationTextComponent("ds.skill.level.cost", skillCost)), mouseX, mouseY, Minecraft.getInstance().screen.width, Minecraft.getInstance().screen.height, 200, Minecraft.getInstance().font);
				}
			}
		});
	}
