...
import by.jackraidenph.dragonsurvival.client.gui.components.BackgroundColorSelectorComponent;
...
public class BackgroundColorButton extends ExtendedButton
{
	...
	public void renderButton(MatrixStack mStack, int mouseX, int mouseY, float partial)
	{
		...
		if(toggled && (!visible || (!isMouseOver(mouseX, mouseY)
		                           && (colorComponent == null || !colorComponent.isMouseOver(mouseX, mouseY))))){
			toggled = false;
			Screen screen = Minecraft.getInstance().screen;
			screen.children.removeIf((s) -> s == colorComponent);
			screen.buttons.removeIf((s) -> s == renderButton);
