...
public class MediumNestBlock extends NestBlock {
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        if (!state.getValue(PRIMARY_BLOCK))
            return DSTileEntities.nestPlaceHolder.create();
    ...
    public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        ...
        NestTileEntity nest = (NestTileEntity) worldIn.getBlockEntity(rootPos);
        ...
        if (dragonStateHandler.isDragon() &&
            ...
            if (Functions.isAirOrFluid(rootPos.relative(playerDirection.getOpposite()), worldIn) &&
                ...
                switch (dragonType) {
                    case SEA:
                        worldIn.setBlockAndUpdate(rootPos, DSBlocks.bigSeaNest.defaultBlockState().setValue(FACING, placementDirection));
                        break;
                    case FOREST:
                        worldIn.setBlockAndUpdate(rootPos, DSBlocks.bigForestNest.defaultBlockState().setValue(FACING, placementDirection));
                        break;
                    case CAVE:
                        worldIn.setBlockAndUpdate(rootPos, DSBlocks.bigCaveNest.defaultBlockState().setValue(FACING, placementDirection));
                        break;
                }
                NestTileEntity nestEntity = getBlockEntity(worldIn, rootPos);
